...
\newcommand*{\seefn}[1]{\hyperref[#1]{\fn{#1}}}
...
The main \lunix module through which most routines and constants are exported. These bindings are type-safe---none operate on or return memory addresses (pointers). For lower-level bindings, see the \seefn{unix.unsafe} submodule.
...
\subsubsection[\fn{access}]{\fn{accept($path$, $mode$)}}

FIXME.

\subsubsection[\fn{alarm}]{\fn{alarm($seconds$)}}
...
\subsubsection[\fn{bitand}]{\fn{bitand($x$, $y$)}}

FIXME.

\subsubsection[\fn{bitor}]{\fn{bitor($x$, $y$)}}
...
\subsubsection[\fn{clearerr}]{\fn{clearerr($fh$)}}
...
\subsubsection[\fn{close}]{\fn{close($fd$)}}
...
\subsubsection[\fn{compl}]{\fn{compl($x$)}}
...
\subsubsection[\fn{faccessat}]{\fn{faccessat($fd$, $path$, $mode$, $flags$)}}

FIXME.

\subsubsection[\fn{fchmod}]{\fn{fchmod($file$, $mode$)}}

See \fn{chmod}.

\subsubsection[\fn{fchown}]{\fn{fchown($file$[, $uid$][, $gid$])}}
...
\subsubsection[\fn{feof}]{\fn{feof($file$)}}

FIXME.

\subsubsection[\fn{ferror}]{\fn{ferror($file$)}}

FIXME.

\subsubsection[\fn{fgetc}]{\fn{fgetc($file$)}}
...
\subsubsection[\fn{fnmatch}]{\fn{fnmatch($pattern$, $subject$, $flags$)}}
...
\subsubsection[\fn{fstatat}]{\fn{fstatat($fd$, $path$, $flags$[, $field$ \ldots])}}

Like \seefn{stat}. See also \seefn{openat}.
...
\subsubsection[\fn{fopenat}]{\fn{fopenat($fd$, $path$[, $mode$][, $perm$])}}

See \syscall{openat} and \syscall{fopen}.

Returns a FILE handle on success, otherwise \nil, an error string, and an integer system error.

\subsubsection[\fn{fpathconf}]{\fn{fpathconf($fd$, $name$)}}
...
\subsubsection[\fn{getc}]{\fn{getc($file$)}}

FIXME.

\subsubsection[\fn{getcwd}]{\fn{getcwd()}}
...
\subsubsection[\fn{getgrgid}]{\fn{getgrgid($gid$[, $\ldots$])}}
...
\subsubsection[\fn{getopt}]{\fn{getopt($args$, $optstring$)}}
...
\subsubsection[\fn{getpwuid}]{\fn{getpwuid($uid$[, $ldots$])}}
...
\subsubsection[\fn{listen}]{\fn{listen($fd$[, $backlog$])}}
...
Identical to \seefn{stat}, except only accepts string paths and uses the \syscall{lstat} system call.
...
\subsubsection[\fn{pathconf}]{\fn{pathconf($path$, $name$)}}
...
\subsubsection[\fn{readlink}]{\fn{readlink($path$)}}

FIXME.

\subsubsection[\fn{readlinkat}]{\fn{readlinkat($fd$, $path$)}}

FIXME.

\subsubsection[\fn{realpath}]{\fn{realpath($path$)}}
...
\subsubsection[\fn{setsockopt}]{\fn{setsockopt($fd$, $level$, $optname$, $optval$)}}
...
\subsubsection[\fn{shutdown}]{\fn{shutdown($fd$, $how$)}}
...
\subsubsection[\fn{socket}]{\fn{socket($family$, $socktype$, $protocol$)}}

FIXME.

\subsubsection[\fn{socketpair}]{\fn{socketpair($family$, $socktype$, $protocol$)}}
...
\subsubsection[\fn{symlinkat}]{\fn{symlinkat($path1$, $atfd$, $path2$)}}

Like \seefn{symlink}, but $path2$ is resolved relative to $atfd$.

\subsubsection[\fn{sysconf}]{\fn{sysconf($name$)}}
...
\subsubsection[\fn{tcgetsid}]{\fn{tcgetsid($fd$)}}
...
\subsubsection[\fn{xor}]{\fn{xor($x$, $y$)}}
...
\begin{Module}{unix.unsafe}

\label{unix.unsafe}

The \module{unix.unsafe} module binds routines that can take or return pointers.
Pointer arguments might be taken directly as lightuserdata or integers (intptr\_t), or indirectly as a string, depending on the routine. In the latter case, if it's not known whether a syscall might write through a pointer (e.g. \fn{fcntl}, \fn{getsockopt}), a string argument will be copied into an internal buffer, a pointer to this buffer passed to the routine, and then the contents of this buffer (possibly modified) returned as a string to the caller in the return list; otherwise (e.g. \fn{strlen}) a pointer to the string obtained via \fn{lua\_tostring} is passed.

\subsubsection[\fn{calloc}]{\fn{calloc($count$, $size$)}}

FIXME

\subsubsection[\fn{fcntl}]{\fn{fcntl($fd$, $cmd$[, $arg$])}}

FIXME

\subsubsection[\fn{fmemopen}]{\fn{fmemopen($addr$, $size$, $mode$)}}

FIXME

\subsubsection[\fn{free}]{\fn{free($addr$)}}

FIXME

\subsubsection[\fn{getsockopt}]{\fn{getsockopt($fd$, $level$, $optname$, $arg$)}}

FIXME

\subsubsection[\fn{ioctl}]{\fn{ioctl($fd$, $cmd$, $arg$)}}

FIXME

\subsubsection[\fn{malloc}]{\fn{malloc($size$)}}

FIXME

\subsubsection[\fn{memcpy}]{\fn{memcpy($dst$, $src$, $size$)}}

FIXME

\subsubsection[\fn{memset}]{\fn{memset($addr$, $c$, $len$)}}

FIXME

\subsubsection[\fn{mlock}]{\fn{mlock($addr$, $len$)}}

FIXME

\subsubsection[\fn{mlockall}]{\fn{mlockall($flags$)}}

FIXME

\subsubsection[\fn{mmap}]{\fn{mmap($addr$, $len$, $prot$, $flags$, $fd$, $offset$)}}

FIXME

\subsubsection[\fn{munlock}]{\fn{munlock($addr$, $len$)}}

FIXME

\subsubsection[\fn{munlockall}]{\fn{munlockall()}}

FIXME

\subsubsection[\fn{munmap}]{\fn{munmap($addr$, $len$)}}

FIXME

\subsubsection[\fn{realloc}]{\fn{realloc($addr$, $size$)}}

FIXME

\subsubsection[\fn{reallocarray}]{\fn{reallocarray($addr$, $count$, $size$)}}

FIXME

\subsubsection[\fn{setsockopt}]{\fn{setsockopt($fd$, $level$, $optname$, $arg$)}}

FIXME

\subsubsection[\fn{strlen}]{\fn{strlen($addr$)}}

FIXME

\subsubsection[\fn{strnlen}]{\fn{strlen($addr$, $maxlen$)}}
